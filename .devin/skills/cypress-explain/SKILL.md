...
- Specs em `e2e/cypress/e2e/` (uma por teste, cabecalho Gherkin PT-PT); helpers em
  `e2e/cypress/support/commands.ts`; config em `e2e/cypress.config.ts`.
- A app e Flutter Web em canvas: elementos aparecem como `flt-semantics` (texto no
  conteudo do no, nao em aria-label) e text fields como
  `<input data-semantics-role="text-field" aria-label="...">`. Titulos de AppBar sao
  `<h2>` reais. Ao explicar falhas, considera rebuilds da arvore de semantica em
  transicoes de rota (causa classica de nos stale).
- Cypress 16: publico -> `Cypress.expose()`; sensivel -> `cy.env(['KEY'])`.
