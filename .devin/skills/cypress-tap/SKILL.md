...
- Invoca o tap SEMPRE via o launcher do projeto, a partir de `e2e/`:
  `cd e2e && node scripts/cypress.js tap <command>` — nunca `npx cypress tap`
  (a variavel global `ELECTRON_RUN_AS_NODE` desta maquina quebra o Electron;
  o launcher remove-a).
- A sessao alvo e o `cypress open` da suite (arranca com `npm run test:open`;
  Electron ou Chrome — ambos suportados pelo tap).
- O AUT e Flutter Web em canvas: o DOM util para `dom`/`aria`/`inspect` e a arvore
  `flt-semantics` (texto no conteudo do no; inputs com
  `data-semantics-role="text-field"` + `aria-label`). Ao procurar seletores para
  specs novas, prioriza os helpers `semNode`/`tapButton`/`fillField` de
  `cypress/support/commands.ts` sobre seletores genericos.
