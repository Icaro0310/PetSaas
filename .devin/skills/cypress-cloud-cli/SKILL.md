...
- Este projeto NAO usa Cypress Cloud (sem `CYPRESS_RECORD_KEY` nem projeto Cloud
  configurado — os runs sao locais + GitHub Actions). Se o utilizador pedir triage
  de runs Cloud, confirma primeiro que `cy-cloud status` autentica; caso contrario,
  reporta o prerequisito em falta e propoe `cy-cloud login` ou analise dos
  artefactos do workflow `.github/workflows/website-e2e.yml` (screenshots/videos
  uploaded como `cypress-artifacts`).
